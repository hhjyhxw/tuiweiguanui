<template>
	<view class="container">
		...
		<view class="wrapper">
			...
			<button v-if="!loginType" class="confirm-btn" @click="chooseLoginType('wechat')" :disabled="logining">微信快速登录</button>
							...
							if (res.code!=null && res.code!='') {
								uni.getUserInfo({
								  ...
								  success: function(res2) {
									...
									var data = {
										appid:that.$api.defConfig().appid,
									...
									that.logining = false
									that.$api.request('wxuser', 'login',data, failres => {
										that.$api.msg(failres.errmsg)
										uni.hideLoading()
									}).then(res => {
										console.log("res====="+JSON.stringify(res));
										uni.setStorageSync('userInfo', res.user)
										that.$store.commit('login', res.user)
										that.$api.setUserInfo(res.user)
...
<style lang='scss'>
	...
	.confirm-btn {
		...
		background:linear-gradient(to right, #B100FF,#00B3FF) !important; color:#FFFFFF !important;
		color: #fff;
		font-size: $font-lg;
