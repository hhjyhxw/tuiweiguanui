		...
		<view slot="gBody" class="grace-body"  >
			<view class="content">
				...
				<view class="ls-grid-3">
					...
					<view class="ls-cell ls-cell-rborder ls-cell-lborder">数量</view>
					<view class="ls-cell">金额</view>
				</view>
				<view class="ls-grid-3" v-for="(item,index) in list">
					<view class="ls-cell">{{item.title}}</view>
...
import { mapState } from 'vuex';  
	export default {
		...
		onShow(){
			this.getGoodsReportList(this.queryData);
		},
		methods: {
			//前一天
			prePage(){
			...
			async getGoodsReportList (querData,first) {
				let result = await this.$api.request('shopkeeper/shopStatistic', 'goodsDayList',querData);
				console.log(JSON.stringify(result));
				if(result.code == 0){
					this.queryData.datetime=result.shopGoodsDayreportdto.datetime;
					this.queryData.formatdatetime=result.shopGoodsDayreportdto.formatdatetime;
				}
		...
		onPullDownRefresh() {
			 this.queryData.pageNum = 1;
			 this.queryData.totalPage = 0;
			 this.getGoodsReportList()(this.queryData,true);
		 ...
		 onReachBottom(){//页面滚动到底部的事件
			if (this.queryData.pageNum > this.queryData.totalPage) {
			 ...
			 this.getGoodsReportList(this.queryData,false);
		 }
	}
</script>
